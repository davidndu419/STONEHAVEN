...
export function SupportPage() {
  ...
  async function create(event) { event.preventDefault(); const first = { id: `msg-${Date.now()}`, senderId: user.userId, senderName: user.name, senderRole: "user", message: form.message, createdAt: new Date().toISOString() }; await dataService.create("supportTickets", { ticketId: ticketId(), userId: user.userId, userName: user.name, adminId: user.adminId, subject: form.subject, category: form.category, status: "open", messages: [first], adminUnread: true, userUnread: false }); setOpen(false); setForm({ subject: "", category: "Account", message: "" }); load(); }
  async function sendReply(event) { event.preventDefault(); if (!reply.trim()) return; const messages = [...(selected.messages || []), { id: `msg-${Date.now()}`, senderId: user.userId, senderName: user.name, senderRole: "user", message: reply, createdAt: new Date().toISOString() }]; await dataService.update("supportTickets", selected.id, { messages, adminUnread: true, updatedAt: new Date().toISOString() }); setReply(""); load(); setSelected({ ...selected, messages }); }
  return <div><PageHeader eyebrow="Human client service" title="Support center" description="Secure, real-human assistance from your assigned Stonehaven team." action={<button onClick={() => setOpen(true)} className="btn-primary"><MessageSquare size={16} /> Open ticket</button>} /><div className="grid gap-6 lg:grid-cols-[.75fr_1.25fr]"><div className="space-y-3">{tickets.map((ticket) => <button key={ticket.id} onClick={async () => { setSelected(ticket); await dataService.update("supportTickets", ticket.id, { userUnread: false }); }} className={`glass-card w-full p-5 text-left ${selected?.id === ticket.id ? "border-gold" : ""}`}><div className="flex justify-between gap-3"><p className="font-bold text-navy">{ticket.subject}</p><StatusBadge status={ticket.status} /></div><p className="mt-2 text-xs text-slate-400">{ticket.ticketId} · {ticket.category}</p></button>)}{!tickets.length && <EmptyState icon={LifeBuoy} title="No support tickets" text="Open a ticket when you need assistance from the Stonehaven team." />}</div><div className="glass-card flex min-h-[520px] flex-col overflow-hidden">{selected ? <><div className="border-b border-slate-200 p-5"><p className="font-display text-2xl font-bold text-navy">{selected.subject}</p><p className="text-xs text-slate-400">{selected.ticketId}</p></div><div className="flex-1 space-y-4 overflow-y-auto p-5">{selected.messages?.map((message) => <div key={message.id} className={`flex ${message.senderRole === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${message.senderRole === "user" ? "bg-navy text-white" : "bg-stone text-slate-700"}`}><p>{message.message}</p><p className={`mt-2 text-[9px] ${message.senderRole === "user" ? "text-white/40" : "text-slate-400"}`}>{message.senderName} · {dateTime(message.createdAt)}</p></div></div>)}</div><form onSubmit={sendReply} className="flex gap-2 border-t border-slate-200 p-4"><input className="field" value={reply} onChange={(e) => setReply(e.target.value)} placeholder="Write a reply..." /><button className="btn-primary px-4"><Send size={17} /></button></form></> : <div className="grid flex-1 place-items-center text-center"><div><LifeBuoy className="mx-auto text-slate-300" size={38} /><p className="mt-3 text-sm text-slate-400">Select a ticket to view the conversation.</p></div></div>}</div></div><Modal open={open} onClose={() => setOpen(false)} title="Open support ticket"><form onSubmit={create} className="space-y-4"><div><label className="label">Subject</label><input className="field" required value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} /></div><div><label className="label">Category</label><select className="field" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}><option>Account</option><option>Deposit</option><option>Withdrawal</option><option>Investment</option><option>KYC</option><option>Technical</option></select></div><div><label className="label">Message</label><textarea className="field min-h-32" required value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} /></div><button className="btn-primary w-full">Create ticket</button></form></Modal></div>;
}
