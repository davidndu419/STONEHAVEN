...
export function AdminSupportInbox() {
  ...
  async function send(event) {
    ...
    const messages = [...(selected.messages || []), {
      ...
      senderRole: user.role, senderPhotoUrl: user.profilePhotoUrl || "", message: reply.trim(), createdAt: new Date().toISOString(),
    }];
  ...
  return <div>
    ...
    <div className={`grid gap-6 ${selected ? "lg:grid-cols-[.7fr_1.3fr]" : ""}`}>
      ...
      {selected && <div className="glass-card flex min-h-[540px] flex-col overflow-hidden">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-5">{selected.messages?.map((message) => <div key={message.id} className={`flex items-end gap-2 ${message.senderRole === "user" ? "justify-start" : "justify-end"}`}>{message.senderRole === "user" && (message.senderPhotoUrl || selected.userPhotoUrl ? <img src={message.senderPhotoUrl || selected.userPhotoUrl} alt="" className="h-8 w-8 rounded-full object-cover" /> : <span className="grid h-8 w-8 place-items-center rounded-full bg-stone text-xs font-bold text-navy">{message.senderName?.charAt(0)}</span>)}<div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm ${message.senderRole === "user" ? "bg-stone text-slate-700" : "bg-navy text-white"}`}><p className="mb-1 text-[10px] font-bold uppercase tracking-wider opacity-60">{message.senderName} · {message.senderRole}</p><p>{message.message}</p><p className="mt-2 text-[9px] opacity-50">{messageStamp(message.createdAt)}</p></div>{message.senderRole !== "user" && (message.senderPhotoUrl ? <img src={message.senderPhotoUrl} alt="" className="h-8 w-8 rounded-full object-cover" /> : <span className="grid h-8 w-8 place-items-center rounded-full bg-navy text-xs font-bold text-gold">{message.senderName?.charAt(0)}</span>)}</div>)}</div>
